...
import com.destrostudios.grid.components.spells.range.*;
...
public class SampleDataGenarator {
  ...
  public static void initTestMap(EntityData data) {
    ...
    for (int x = 0; x < MAP_X; x++) {
      for (int y = 0; y < MAP_Y; y++) {
        ...
        boolean isWalkableAndNoStartingField =
            walkableEntities.stream()
        ...
        if (isWalkableAndNoStartingField && Math.random() < 0.2) {
          ...
          data.addComponent(
              treeComponent, new VisualComponent(Math.random() > 0.5 ? "tree" : "rock"));
  ...
  public static void initTestCharacter(EntityData data, String charackterName) {
    ...
    addDmgSpell(data, rand, attackPoints, spell, SpellAreaShape.ORTHOGONAL_LINE);
    spells.add(spell);

    int spell2 = data.createEntity();
    addDmgSpell(data, rand, attackPoints, spell2, SpellAreaShape.SQUARE);
    spells.add(spell2);

    int spell3 = data.createEntity();
    addDmgSpell(data, rand, attackPoints, spell3, SpellAreaShape.PLUS);
    spells.add(spell3);
  ...
  private static void addDmgSpell(
      EntityData data, Random rand, int attackPoints, int spell, SpellAreaShape indicator) {
    ...
    int range = 6;
    data.addComponent(spell, new DamageComponent(dmg, dmg + 50, true));
    ...
    data.addComponent(
        ...
        new TooltipComponent(
            String.format(
                "OP spell doing %s damage for %s AP in %s", Math.abs(dmg), apCost, indicator)));
    data.addComponent(spell, new CastsPerTurnComponent(3, 0));
    data.addComponent(spell, new RangeComponent(CastAreaShape.DIAMOND, 1, range));
    data.addComponent(spell, new AffectedAreaComponent(indicator, 0, 4));
  }

  private static void addDmgSpellWithMpBuff(
      EntityData data, Random rand, int attackPoints, int dmgMpSpell) {
    int apCost = Math.max(2, rand.nextInt(attackPoints));
    int dmg = Math.max(25, rand.nextInt(50));
    ...
    data.addComponent(dmgMpSpell, new DamageComponent(dmg, dmg + 30, true));
    data.addComponent(dmgMpSpell, new NameComponent(spellName));
    data.addComponent(dmgMpSpell, new MovementPointBuffComponent(mpBuff, 1, BuffType.PLAYER));
    data.addComponent(dmgMpSpell, new RangeComponent(CastAreaShape.DIAMOND, 1, range));
    data.addComponent(dmgMpSpell, new CastsPerTurnComponent(2, 0));
    data.addComponent(
        dmgMpSpell,
        new TooltipComponent(
            String.format(
                "Dmg spell doing %s dmg for %s AP and buffing %s MP\nRange: %s",
                dmg, apCost, mpBuff, range)));
  }
  ...
  private static void addTeleport(EntityData data, Random rand, int attackPoints, int spellApBuff) {
    ...
    data.addComponent(spellApBuff, new RangeComponent(CastAreaShape.DIAMOND, 1, range));
    data.addComponent(spellApBuff, new BuffsComponent(new ArrayList<>()));
  ...
  private static void addApAndHpPoison(
      EntityData data, Random rand, int attackPoints, int spell, int playerEntity) {
    int range;
    int apCost = Math.max(2, rand.nextInt(attackPoints));
    range = Math.max(3, rand.nextInt(6));
    data.addComponent(spell, new CostComponent(apCost, 0, 0));
    data.addComponent(spell, new AttackPointsPerTurnComponent(-1, -3, 2));
    data.addComponent(spell, new DamagePerTurnComponent(-50, -100, 3));
    ...
    data.addComponent(
        spell,
        new TooltipComponent(
            "Spell adds 1-3 AP poison for 2 rounds and 50-100 HP poison for 3 rounds"));
  }

  private static void addHealthBuff(
      EntityData data, Random rand, int attackPoints, int spellMpHealthBuff) {
    int apCost;
    ...
    data.addComponent(
        spellMpHealthBuff, new HealthPointBuffComponent(hpBuff, hpBuffDuration, BuffType.PLAYER));
    ...
    data.addComponent(spellMpHealthBuff, new RangeComponent(CastAreaShape.DIAMOND, 0, 0));
    data.addComponent(
        spellMpHealthBuff,
        new TooltipComponent(
            String.format(
                "Spell buffing %s HP for %s AP. \nCD: %s, Range: 0 ", hpBuff, apCost, cooldown)));
  ...
  private static void addHeal(EntityData data, int spell, int playerEntity) {
    ...
    data.addComponent(spell, new CostComponent(apCost, 0, 0));
    data.addComponent(spell, new HealPerTurnComponent(50, 100, 4));
    data.addComponent(spell, new NameComponent("Blblbl"));
    data.addComponent(spell, new RangeComponent(CastAreaShape.DIAMOND, 0, 3));
    data.addComponent(spell, new CastsPerTurnComponent(3, 0));
    data.addComponent(spell, new BuffsComponent(new ArrayList<>()));
    data.addComponent(
        spell, new TooltipComponent(String.format("Spell heals 50-100 hp for %s rounds", 4)));
  }
  ...
  private static void addDisplacement(EntityData data, int spell, int playerEntity) {
    ...
    data.addComponent(spell, new CastsPerTurnComponent(3, 0));
    data.addComponent(spell, new LineOfSightComponent());
    data.addComponent(spell, new RangeComponent(CastAreaShape.DIAMOND, 1, 1));
    data.addComponent(spell, new BuffsComponent(new ArrayList<>()));
    data.addComponent(spell, new PushComponent(5, false));
    data.addComponent(
        spell, new TooltipComponent(String.format("Displaces player 5 positions for 4 AP")));
  }

  private static String getSpellName() {
    List<String> spellName =
        Lists.newArrayList(
            "Arrow", "Hit", "Jump", "Twist", "Confusion", "Nothing", "Blblbl", "Wound");
