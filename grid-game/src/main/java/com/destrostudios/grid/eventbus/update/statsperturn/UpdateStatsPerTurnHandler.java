...
public class UpdateStatsPerTurnHandler implements EventHandler<UpdateStatsPerTurnEvent> {
  ...
  private List<Event> handlePoisons(EntityData entityData, int playerEntity) {
    ...
    StatsPerRoundComponent statsPerTurnComp =
        entityData.getComponent(playerEntity, StatsPerRoundComponent.class);
    ...
    for (int statsPerTurn : statsPerTurnComp.getStatsPerRoundEntites()) {
      ...
      if (entityData.hasComponents(statsPerTurn, AttackPointsPerTurnComponent.class)) {
        AttackPointsPerTurnComponent apPoison =
            entityData.getComponent(statsPerTurn, AttackPointsPerTurnComponent.class);
        AttackPointResistanceComponent apResSource =
            entityData.getComponent(source.getSourceEntity(), AttackPointResistanceComponent.class);
        AttackPointResistanceComponent apResTarget =
            entityData.getComponent(playerEntity, AttackPointResistanceComponent.class);
        apPoisonSum +=
            getResultingValue(
                apPoison.getMaxValue() - apPoison.getMinValue(),
                apResSource.getResistanceValue(),
                apResTarget.getResistanceValue());
        ...
        MovementPointsPerTurnComponent mpPerTurn =
            entityData.getComponent(statsPerTurn, MovementPointsPerTurnComponent.class);
        MovementPointResistanceComponent mpResSource =
            entityData.getComponent(
                source.getSourceEntity(), MovementPointResistanceComponent.class);
        MovementPointResistanceComponent mpResTarget =
            entityData.getComponent(playerEntity, MovementPointResistanceComponent.class);
        mpPoisonSum +=
            getResultingValue(
                mpPerTurn.getMaxValue() - mpPerTurn.getMinValue(),
                mpResSource.getResistanceValue(),
                mpResTarget.getResistanceValue());

      } else if (entityData.hasComponents(statsPerTurn, HealPerTurnComponent.class)) {
        HealPerTurnComponent heal =
            entityData.getComponent(statsPerTurn, HealPerTurnComponent.class);
        ...
        DamagePerTurnComponent damage =
            entityData.getComponent(statsPerTurn, DamagePerTurnComponent.class);
        hpPoisonSum += random.nextInt(damage.getMinValue(), damage.getMaxValue());
      }
    }

    if (apPoisonSum != 0) {
      AttackPointsComponent component =
          entityData.getComponent(playerEntity, AttackPointsComponent.class);
      followUpEvents.add(
          new AttackPointsChangedEvent(playerEntity, component.getAttackPoints() + apPoisonSum));
    }
    if (mpPoisonSum != 0) {
      MovementPointsComponent component =
          entityData.getComponent(playerEntity, MovementPointsComponent.class);
      followUpEvents.add(
          new MovementPointsChangedEvent(
              playerEntity, component.getMovementPoints() + mpPoisonSum));
    }
    if (hpPoisonSum != 0) {
      HealthPointsComponent component =
          entityData.getComponent(playerEntity, HealthPointsComponent.class);
      followUpEvents.add(
          new HealthPointsChangedEvent(playerEntity, component.getHealth() + hpPoisonSum));
    }
    return followUpEvents;
  }

  private int getSourceEntity(
      EntityData data, int spellEntity, int playerEntity) { // every spell is unique
    List<Integer> spellsEntities =
        data.list(SpellsComponent.class).stream()
