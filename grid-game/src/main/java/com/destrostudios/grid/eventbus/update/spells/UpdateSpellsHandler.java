...
import com.destrostudios.grid.components.spells.buffs.AntiCCBuffComponent;
...
public class UpdateSpellsHandler implements EventHandler<UpdateSpellsEvent> {
  ...
  public void onEvent(UpdateSpellsEvent event, Supplier<EntityData> entityDataSupplier) {
    ...
    SpellsComponent spellsComponent =
        entityData.getComponent(event.getEntity(), SpellsComponent.class);
  ...
  private void updateBuff(EntityData entityData, Component component, Integer buffEntity) {
      ...
      if (buffComp.getBuffDuration() == 1) {
        ...
        entityData.addComponent(
            ...
            new AttackPointsBuffComponent(
                buffComp.getBuffAmount(), buffComp.getBuffDuration() - 1, buffComp.getBuffType()));
      }
    }
    ...
    if (component instanceof AntiCCBuffComponent) {
      AntiCCBuffComponent ccBuffComponent = (AntiCCBuffComponent) component;
      if (ccBuffComponent.getBuffDuration() == 1) {
        entityData.remove(buffEntity, AntiCCBuffComponent.class);
      } else {
        entityData.addComponent(
            buffEntity,
  ...
  private void updateCooldown(EntityData entityData, Integer spellEntity) {
    if (entityData.hasComponents(spellEntity, OnCooldownComponent.class)) {
      OnCooldownComponent onCooldownComponent =
          entityData.getComponent(spellEntity, OnCooldownComponent.class);
      if (onCooldownComponent.getRemainingRounds() == 1) {
        entityData.remove(spellEntity, OnCooldownComponent.class);
      } else {
        entityData.addComponent(
            spellEntity, new OnCooldownComponent(onCooldownComponent.getRemainingRounds() - 1));
      }
    }
  }
  ...
  private void updateCastsPerTurn(EntityData entityData, Integer spellEntity) {
    ...
    if (entityData.hasComponents(spellEntity, CastsPerTurnComponent.class)) {
      entityData.addComponent(
          spellEntity, new CastsPerTurnComponent(castsPerTurnComponent.getMaxCastsPerTurn(), 0));
    }
  }
}
