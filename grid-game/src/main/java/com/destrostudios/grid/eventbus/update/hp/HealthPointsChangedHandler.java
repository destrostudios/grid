...
public class HealthPointsChangedHandler implements EventHandler<HealthPointsChangedEvent> {
  ...
  public void onEvent(HealthPointsChangedEvent event, Supplier<EntityData> entityDataSupplier) {
    ...
    MaxHealthComponent maxHealthComponent =
        entityData.getComponent(event.getEntity(), MaxHealthComponent.class);
    HealthPointsComponent hpComponent =
        new HealthPointsComponent(
            Math.min(maxHealthComponent.getMaxHealth(), event.getNewPoints()));
    entityData.addComponent(event.getEntity(), hpComponent);

    if (hpComponent.getHealth() <= 0) {
      Event followUpEvent = new DieEvent(event.getEntity());
      eventbus.registerSubEvents(followUpEvent);
    }
  }
}
