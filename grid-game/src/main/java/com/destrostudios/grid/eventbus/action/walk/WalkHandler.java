...
public class WalkHandler implements EventHandler<WalkEvent> {
  private static final Logger logger = Logger.getLogger(WalkHandler.class.getSimpleName());
  ...
  public void onEvent(WalkEvent componentUpdateEvent, Supplier<EntityData> supplier) {
    ...
    List<Event> events =
        Lists.newArrayList(
            new MoveEvent(entity, componentUpdateEvent.getPositionComponent(), WALK),
