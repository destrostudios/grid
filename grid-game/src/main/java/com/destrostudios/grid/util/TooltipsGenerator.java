...
public class TooltipsGenerator {
    ...
    public static TooltipComponent generateTooltip(EntityData entityData, int spell) {
        String tooltip = getPrefix(entityData, spell)
    ...
    private static String getUtilityString(EntityData entityData, int spell) {
        ...
        if (entityData.hasComponents(spell, PullComponent.class)) {
            ...
            String string = component.isUseTargetAsOrigin()
    ...
    private static String getRestrictionsString(EntityData entityData, int spell) {
        ...
        String rangeString = range.getMinRange() == range.getMaxRange()
                ? "range: " + range.getMinRange()
                : "range: " + range.getMinRange() + " - " + range.getMaxRange();
        restrictions.add(rangeString);

        String lineOfSight = entityData.hasComponents(spell, LineOfSightComponent.class)
                ? "needs line of sight"
                : "no line of sight" ;
    ...
    private static String getStatsPerTurnString(EntityData entityData, int spell) {
        ...
        if (entityData.hasComponents(spell, DamagePerTurnComponent.class)) {
            DamagePerTurnComponent dmgPerTurn = entityData.getComponent(spell, DamagePerTurnComponent.class);
            statsPerTurn.add(String.format("%s-%s dmg per turn for %s rounds", dmgPerTurn.getMinValue(), dmgPerTurn.getMaxValue(), dmgPerTurn.getDuration()));
        }
        if (entityData.hasComponents(spell, HealPerTurnComponent.class)) {
            ...
            statsPerTurn.add(String.format("%s-%s heal per turn for %s rounds", healPerTurn.getMinValue(), healPerTurn.getMaxValue(), healPerTurn.getDuration()));
        }
        if (entityData.hasComponents(spell, AttackPointsPerTurnComponent.class)) {
            AttackPointsPerTurnComponent apPerTurn = entityData.getComponent(spell, AttackPointsPerTurnComponent.class);
            statsPerTurn.add(String.format("%s-%s AP per turn for %s rounds", apPerTurn.getMinValue(), apPerTurn.getMaxValue(), apPerTurn.getDuration()));
        }
        if (entityData.hasComponents(spell, MovementPointsPerTurnComponent.class)) {
            MovementPointsPerTurnComponent mpPerTurn = entityData.getComponent(spell, MovementPointsPerTurnComponent.class);
            statsPerTurn.add(String.format("%s-%s MP per turn for %s rounds", mpPerTurn.getMinValue(), mpPerTurn.getMaxValue(), mpPerTurn.getDuration()));
        }

        return !statsPerTurn.isEmpty()
                ? "Buffs " + String.join(", ", statsPerTurn) + ". "
                : "";
    }
    ...
    private static String getBuffString(EntityData entityData, int spell) {
        ...
        if (entityData.hasComponents(spell, AttackPointsBuffComponent.class)) {
            AttackPointsBuffComponent apBuff = entityData.getComponent(spell, AttackPointsBuffComponent.class);
        ...
        if (entityData.hasComponents(spell, MovementPointBuffComponent.class)) {
            MovementPointBuffComponent mpBuff = entityData.getComponent(spell, MovementPointBuffComponent.class);
        ...
        if (entityData.hasComponents(spell, HealthPointBuffComponent.class)) {
            HealthPointBuffComponent hpBuff = entityData.getComponent(spell, HealthPointBuffComponent.class);
            buffs.add(hpBuff.getBuffAmount() + " health points for " + hpBuff.getBuffDuration() + " rounds");
        }
        if (entityData.hasComponents(spell, ReflectionBuffComponent.class)) {
            ReflectionBuffComponent reflection = entityData.getComponent(spell, ReflectionBuffComponent.class);
            buffs.add(reflection.getBuffAmount() + " reflection for " + reflection.getBuffDuration() + " rounds");
        }
