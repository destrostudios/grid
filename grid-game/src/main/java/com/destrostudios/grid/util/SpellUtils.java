...
public class SpellUtils {
  ...
  public static List<Integer> getAffectedTargetableEntities(
      int spellEntity,
      int sourceEntity,
      PositionComponent sourcePos,
      PositionComponent clickedPos,
      EntityData entityData) {

    Set<PositionComponent> positionComponents =
        calculateAffectedPositions(
            sourcePos,
            clickedPos,
            entityData.getComponent(spellEntity, AffectedAreaComponent.class),
            entityData);
    int teamSource = entityData.getComponent(sourceEntity, TeamComponent.class).getTeam();

    return entityData.list(TargetableComponent.class, PositionComponent.class).stream()
        .filter(
  ...
  private static Comparator<Integer> entitiesToDamageSortComparator(
      PositionComponent clickedPos, EntityData entityData, int teamSource) {
    return Comparator.comparingInt(
        ...
        .thenComparingInt(
            (Integer entity) -> {
              ...
              return -Math.abs(
                  teamSource - entityData.getComponent(entity, TeamComponent.class).getTeam());
            })
        .thenComparingInt(
            (Integer entity) -> {
  ...
  public static List<Integer> getAffectedWalkableEntities(
      int spellEntity,
      PositionComponent sourcePos,
      PositionComponent clickedPos,
      EntityData entityData) {
  ...
  public static Set<PositionComponent> calculateAffectedPositions(
      PositionComponent sourcePos,
      PositionComponent clickedPos,
      AffectedAreaComponent component,
      EntityData entityData) {
    ...
    if (component.getShape() == SpellAreaShape.MAP) {
      List<Integer> entities = entityData.list(PositionComponent.class);
      return entities.stream()
          .map(e -> entityData.getComponent(e, PositionComponent.class))
          .collect(Collectors.toCollection(LinkedHashSet::new));
    } else if (component.getShape() == SpellAreaShape.SINGLE) {
      return Set.of(clickedPos);

    } else if (component.getShape() == SpellAreaShape.LINE) {
      return calculateAffectedPosEntitiesForVerticalLine(
          sourcePos, clickedPos, maxImpact, minImpact, yPos, xPos);

    } else if (component.getShape() == SpellAreaShape.ORTHOGONAL_LINE) {
      return calculateAffectedPosEntitiesForHorizontalLine(
          sourcePos, clickedPos, maxImpact, minImpact, yPos, xPos);
      ...
      if (component.getShape() == SpellAreaShape.PLUS) {
        ...
        getBaseSquare(clickedPos, minImpact - 1).forEach(result::remove);

      } else if (component.getShape() == SpellAreaShape.DIAMOND) {
        result.removeIf(
            pos ->
                Math.abs(pos.getX() - xPos) + Math.abs(pos.getY() - yPos) > maxImpact
  ...
  private static Set<PositionComponent> calculateAffectedPosEntitiesForVerticalLine(
      PositionComponent sourcePos,
      PositionComponent clickedPos,
      int maxImpact,
      int minImpact,
    ...
    Function<Integer, Boolean> testY =
        signumY < 0 ? y -> y >= yPos - maxImpact : y -> y <= yPos + maxImpact;

    int signumX = (int) Math.signum(clickedPos.getX() - sourcePos.getX());
    Function<Integer, Boolean> testX =
        signumX < 0 ? x -> x >= xPos - maxImpact : x -> x <= xPos + maxImpact;

    Set<PositionComponent> result = new LinkedHashSet<>();

    if (Math.abs(sourcePos.getX() - clickedPos.getX())
        < Math.abs(sourcePos.getY() - clickedPos.getY())) {
    ...
    } else if (Math.abs(sourcePos.getX() - clickedPos.getX())
        > Math.abs(sourcePos.getY() - clickedPos.getY())) {
      // from left or right
      ...
      // diagonal
      for (int x = xPos + signumX * minImpact, y = yPos + signumY * minImpact;
          testX.apply(x) && testY.apply(y);
          x += signumX, y += signumY) {
  ...
  private static Set<PositionComponent> calculateAffectedPosEntitiesForHorizontalLine(
      PositionComponent sourcePos,
      PositionComponent clickedPos,
      int maxImpact,
      int minImpact,
    ...
    int deltaX = Math.abs(sourcePos.getX() - clickedPos.getX());
    int deltaY = Math.abs(sourcePos.getY() - clickedPos.getY());
    int impactDelta = maxImpact - minImpact;

    if (deltaX <= deltaY) {
      // Delta x <= delta Y => Horizontal
      for (int x = xPos - maxImpact; x <= xPos + maxImpact; x++) {
        if (x >= xPos - impactDelta && x <= xPos + impactDelta) {
          result.add(new PositionComponent(x, yPos));
        }
      }
    } else {
      // Vertical
      for (int y = yPos - maxImpact; y <= yPos + maxImpact; y++) {
        if (y >= yPos - impactDelta && y <= yPos + impactDelta) {
          result.add(new PositionComponent(xPos, y));
        }
      }
    }
  ...
  public static int calculateTargetEntity(int x, int y, EntityData data) {
    Optional<Integer> targetEntity = data.findEntitiesByComponentValue(new PositionComponent(x, y))
            ...
            .min(
                (e1, e2) ->
                    Boolean.compare(
                        data.hasComponents(e2, TargetableComponent.class),
                        data.hasComponents(e1, TargetableComponent.class)));
  ...
  public static boolean isPositionIsFree(
      EntityData entityData, PositionComponent newPosition, int entity) {
    List<Integer> newPositionEntities =
        new ArrayList<>(entityData.findEntitiesByComponentValue(newPosition));
    // TODO: do we really need to explicitly remove entity here? it will only block itself if we try
    // to move it to its own position.
    newPositionEntities.remove((Integer) entity);
    boolean collidesWithOtherPlayer =
        newPositionEntities.stream()
            .anyMatch(e -> entityData.hasComponents(e, PlayerComponent.class));
    boolean collidesWithObstacle =
        newPositionEntities.stream()
            .anyMatch(e -> entityData.hasComponents(e, ObstacleComponent.class));
    boolean isWalkableField =
        newPositionEntities.stream()
  ...
  public static PositionComponent getDisplacementGoal(
      EntityData entityData, int entity, PositionComponent from, Direction direction, int steps) {
    PositionComponent result = from;
    for (int i = 1; i <= steps; i++) {
      PositionComponent next =
          new PositionComponent(
              from.getX() + i * direction.getDeltaX(), from.getY() + i * direction.getDeltaY());
  ...
  public static boolean isCastable(int casterEntity, int spellEntity, EntityData entityData) {
    return isCostPayable(casterEntity, spellEntity, entityData)
        && maxCastsNotReached(spellEntity, entityData);
  }

  public static boolean maxCastsNotReached(int spellEntity, EntityData entityData) {
    CastsPerTurnComponent component =
        entityData.getComponent(spellEntity, CastsPerTurnComponent.class);
  ...
  public static boolean isCostPayable(int casterEntity, int spellEntity, EntityData entityData) {
    ...
    AttackPointsComponent apComp =
        entityData.getComponent(casterEntity, AttackPointsComponent.class);
    MovementPointsComponent mpComp =
        entityData.getComponent(casterEntity, MovementPointsComponent.class);
    HealthPointsComponent hpComp =
        entityData.getComponent(casterEntity, HealthPointsComponent.class);
    return apComp.getAttackPoints() >= cost.getApCost()
        && mpComp.getMovementPoints() >= cost.getMpCost()
        && hpComp.getHealth() >= cost.getHpCost();
  }
