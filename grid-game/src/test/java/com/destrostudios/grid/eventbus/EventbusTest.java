...
class EventbusTest {
            ...
            (event, entityDataSupplier) -> {
              ...
              if (event.name.equals("A")) {
                eventbus.registerSubEvents(
                    new SimpleEvent("A1"), new SimpleEvent("A2"), new SimpleEvent("A3"));
  ...
  void triggeredHandlersInQueue() {
    ...
    EventHandler<Event> preHandler =
        (event, entityDataSupplier) -> {
          eventTracker.set("Pre");
        };
    EventHandler<Event> instantHandler =
        (event, entityDataSupplier) -> {
          eventTracker.set("Instant");
        };
    EventHandler<Event> resolvedHandler =
        (event, entityDataSupplier) -> {
  ...
  public void testEventRemovedWhenTriggering() {
    ...
    eventbus.addInstantHandler(
        SimpleValueEvent.class,
        (EventHandler<SimpleValueEvent>)
            (event, entityDataSupplier) -> {
              valueTracker.addAndGet(event.getValue());
            });

    eventbus.addEventValidator(
        SimpleValueEvent.class,
        (EventValidator<SimpleValueEvent>) (event, supplier) -> valueTracker.get() < 5);
    ...
    Assertions.assertEquals(
        eventA.getValue(),
        valueTracker.get(),
        "Event B should not get execuuted, as its invalid" + " after executing event A");
  }
