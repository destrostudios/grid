...
import com.destrostudios.grid.actions.*;
...
public class NetworkGridService implements GameService<GridGame, Action> {
  ...
  public void initialize(Kryo kryo) {
    ...
    kryo.register(
        ...
        new Serializer<PositionUpdateAction>() {
          @Override
          ...
          @Override
          public PositionUpdateAction read(
              Kryo kryo, Input input, Class<PositionUpdateAction> type) {
            return new PositionUpdateAction(input.readInt(), input.readInt(), input.readString());
          }
        });
    kryo.register(
        SkipRoundAction.class,
        new Serializer<SkipRoundAction>() {
          @Override
    ...
    kryo.register(
        CastSpellAction.class,
        new Serializer<CastSpellAction>() {
          @Override
          ...
          public CastSpellAction read(Kryo kryo, Input input, Class<CastSpellAction> type) {
            return new CastSpellAction(
                input.readInt(), input.readInt(), input.readString(), input.readInt());
          }
        });
    kryo.register(
        SurrenderAction.class,
        new Serializer<SurrenderAction>() {
          @Override
          public void write(Kryo kryo, Output output, SurrenderAction object) {
            output.writeString(object.getPlayerIdentifier());
            output.writeInt(object.getPlayer());
          }

          @Override
          public SurrenderAction read(Kryo kryo, Input input, Class<SurrenderAction> type) {
            return new SurrenderAction(input.readString(), input.readInt());
          }
        });
  }
  ...
  public GridGame applyAction(GridGame state, Action action, NetworkRandom random) {
    if (!(state.getRandom() instanceof MutableRandomProxy)) {
      throw new UnsupportedOperationException(
          "Networking requires a " + MutableRandomProxy.class.getSimpleName());
    }
