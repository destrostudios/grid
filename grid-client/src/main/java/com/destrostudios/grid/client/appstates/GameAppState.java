...
public class GameAppState extends BaseAppState<ClientApplication> implements ActionListener {
  ...
  public void initialize(AppStateManager stateManager, Application application) {
    ...
    gameProxy.addPreHandler(
        MoveEvent.class,
        (EventHandler<MoveEvent>)
            (event, entityDataSupplier) -> {
              EntityVisual entityVisual =
                  getAppState(MapAppState.class).getEntityVisual(event.getEntity());
              PositionComponent positionComponent = event.getPositionComponent();
              if (event.getMoveType() == MoveType.WALK) {
                playAnimation(
                    new WalkAnimation(
                        entityVisual, positionComponent.getX(), positionComponent.getY(), 8));
              } else if (event.getMoveType() != MoveType.TELEPORT) {
                playAnimation(
                    new MoveAnimation(
                        entityVisual, positionComponent.getX(), positionComponent.getY(), 30));
              }
            });
    gameProxy.addPreHandler(
        HealthPointsChangedEvent.class,
        (EventHandler<HealthPointsChangedEvent>)
            (event, entityDataSupplier) -> {
    ...
    gameProxy.addPreHandler(
        SpellCastedEvent.class,
        (EventHandler<SpellCastedEvent>)
            (event, entityDataSupplier) -> {
              EntityData entityData = entityDataSupplier.get();
              EntityVisual entityVisual =
                  getAppState(MapAppState.class).getEntityVisual(event.getPlayerEntity());
              String spellName =
                  entityData.getComponent(event.getSpell(), NameComponent.class).getName();
              CustomBlockingAnimationInfo castAnimation = CastAnimations.get(spellName);
              if (castAnimation != null) {
                lookAt(
                    entityData,
                    event.getPlayerEntity(),
                    entityVisual.getModelObject(),
                    event.getX(),
                    event.getY());
    ...
    mainApplication
        .getInputManager()
        .addMapping("mouse_left", new MouseButtonTrigger(MouseInput.BUTTON_LEFT));
    mainApplication
        .getInputManager()
        .addMapping("mouse_right", new MouseButtonTrigger(MouseInput.BUTTON_RIGHT));
    mainApplication
        .getInputManager()
        .addListener(
            this,
  ...
  private void updateGame() {
    do {
      while (gameProxy.triggeredHandlersInQueue()
          && playingAnimations.stream().noneMatch(Animation::isBlocking)) {
  ...
  private void updateValidAndInvalidGroundEntities() {
    ...
    if (targetingSpellEntity != null) {
      validSpellTargetEntities =
          getAllTargetableEntitiesInRange(
              targetingSpellEntity, gameProxy.getPlayerEntity(), gameProxy.getGame().getData());
      invalidSpellTargetEntities =
          getAllEntitiesInRange(
              targetingSpellEntity, gameProxy.getPlayerEntity(), gameProxy.getGame().getData());
    ...
    getAppState(MapAppState.class)
        .setValidGroundEntities(validSpellTargetEntities, invalidSpellTargetEntities);
  }
  ...
  private void updateImpactedAndReachableGroundEntities() {
    ...
    if ((gameProxy.getPlayerEntity() != null) && (hoveredPosition != null)) {
      ...
      if (targetingSpellEntity != null) {
        ...
        // Reachable
        if (data.hasComponents(gameProxy.getPlayerEntity(), ActiveTurnComponent.class)
            && (!gameProxy.triggeredHandlersInQueue())) {
          Optional<List<Position>> path = findPathToHoveredPosition();
          if (path.isPresent()) {
            reachableGroundEntities.addAll(
                data.list(WalkableComponent.class).stream()
                    .filter(
                        groundEntity -> {
                          PositionComponent positionComponent =
                              data.getComponent(groundEntity, PositionComponent.class);
                          return path.get()
                              .contains(
                                  new Position(positionComponent.getX(), positionComponent.getY()));
                        })
  ...
  private void updateGui() {
    ...
    int activePlayerMP =
        entityData
            .getComponent(activePlayerEntity, MovementPointsComponent.class)
            .getMovementPoints();
    int activePlayerAP =
        entityData.getComponent(activePlayerEntity, AttackPointsComponent.class).getAttackPoints();
    ...
    for (int i = 0; i < guiNextPlayers.length; i++) {
      ...
      PlayerInfo playerInfo =
          gameProxy.getStartGameInfo().getTeam1().stream()
              .filter(currentPlayerInfo -> currentPlayerInfo.getLogin().equals(playerName))
              .findFirst()
              .orElse(
                  gameProxy.getStartGameInfo().getTeam2().stream()
                      .filter(currentPlayerInfo -> currentPlayerInfo.getLogin().equals(playerName))
                      .findFirst()
                      .orElse(null));
      guiNextPlayers[i] = new GuiNextPlayer(playerName, playerInfo.getCharacterName());
      nextPlayerEntity =
          entityData.getComponent(nextPlayerEntity, NextTurnComponent.class).getNextPlayer();
    }
    ...
    int ownPlayerCurrentHealth =
        entityData.getComponent(playerEntity, HealthPointsComponent.class).getHealth();
    int ownPlayerMaximumHealth =
        entityData.getComponent(playerEntity, MaxHealthComponent.class).getMaxHealth();
    int ownPlayerMP =
        entityData.getComponent(playerEntity, MovementPointsComponent.class).getMovementPoints();
    int ownPlayerAP =
        entityData.getComponent(playerEntity, AttackPointsComponent.class).getAttackPoints();
    ...
    List<GuiSpell> guiSpells =
        spells.getSpells().stream()
            .map(
                spellEntity -> {
                  String name = entityData.getComponent(spellEntity, NameComponent.class).getName();
                  String tooltip =
                      entityData.getComponent(spellEntity, TooltipComponent.class).getTooltip();
                  Integer remainingCooldown =
                      entityData.hasComponents(spellEntity, OnCooldownComponent.class)
                          ? entityData
                              .getComponent(spellEntity, OnCooldownComponent.class)
                              .getRemainingRounds()
                          : null;
          ...
          if (hoveredPosition != null) {
            if (targetingSpellEntity != null) {
              if (containsEntity(
                ...
                gameProxy.requestAction(
                    new CastSpellAction(
                        hoveredPosition.getX(),
                        hoveredPosition.getZ(),
                        gameProxy.getPlayerEntity().toString(),
                        targetingSpellEntity));
              }
              setTargetingSpell(null);
            } else {
              ...
              if (path.isPresent()) {
                for (Position step : path.get()) {
                  gameProxy.requestAction(
                      new PositionUpdateAction(step.x, step.y, playerEntity.toString()));
                }
              }
            }
          }
        ...
        default:
          if (actionName.startsWith("key_")) {
            SpellsComponent spellsComponent =
                entityData.getComponent(playerEntity, SpellsComponent.class);
  ...
  private void lookAt(
      EntityData entityData, int sourceEntity, Spatial sourceSpatial, int targetX, int targetY) {
    PositionComponent sourcePositionComponent =
        entityData.getComponent(sourceEntity, PositionComponent.class);
  ...
  private boolean containsEntity(EntityData entityData, List<Integer> entities, int x, int y) {
    return entities.stream()
        .anyMatch(
            entity -> {
              PositionComponent positionComponent =
                  entityData.getComponent(entity, PositionComponent.class);
  ...
  private Optional<List<Position>> findPathToHoveredPosition() {
    ...
    return new Pathfinder()
        .findPath(
            p ->
                SpellUtils.isPositionIsFree(
                    data, new PositionComponent(p.x, p.y), gameProxy.getPlayerEntity()),
            new Position(playerPosition.getX(), playerPosition.getY()),
            new Position(hoveredPosition.getX(), hoveredPosition.getZ()),
            data.getComponent(gameProxy.getPlayerEntity(), MovementPointsComponent.class)
                .getMovementPoints());
  }
